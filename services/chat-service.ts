...
export async function fetchChatList(userId: string): Promise<ChatListItem[]> {
  // Three plain single-table queries instead of nested joins. The
  // hand-written Database type has no Relationships metadata for the
  // query builder to infer `chats(...)` / `profiles(...)` join shapes
  // from — that metadata only exists in output generated by
  // `supabase gen types`. Splitting into separate queries keeps each one
  // trivially well-typed; RLS still applies identically to each.
  ...
  chats?.forEach((c) => chatById.set(c.id, c as Chat));
  ...
  const { data: profiles, error: profilesErr } = otherUserIds.length
